...
DST_THRESHOLD_LOW = -90
DST_THRESHOLD_MODERATE = -110
DST_THRESHOLD_INTENSE = -200
DST_THRESHOLD_SUPERINTENSE = -380
...
def plot_comparison_bfe(
    ...
    if plot_asy_bars:
        max_asy = roundup(df[index_column_name].max())
        if max_asy >= ASY_H_THRESHOLD_LOW:
            ax.axvline(ASY_H_THRESHOLD_LOW, linestyle="--", color=COLOR_LOW)
            if max_asy >= ASY_H_THRESHOLD_MODERATE:
                ax.axvline(
                    ASY_H_THRESHOLD_MODERATE,
                    linestyle="--",
                    color=COLOR_MODERATE,
                )
                if max_asy >= ASY_H_THRESHOLD_INTENSE:
                    ax.axvline(
                        ASY_H_THRESHOLD_INTENSE,
                        linestyle="--",
                        color=COLOR_INTENSE,
                    )
                    if max_asy >= ASY_H_THRESHOLD_SUPERINTENSE:
                        ax.axvline(
                            ASY_H_THRESHOLD_SUPERINTENSE,
                            linestyle="--",
                            color=COLOR_SUPERINTENSE,
                        )
...
def plot_evaluation_bfe(
    ...
    if plot_asy_bars:
        max_asy = roundup(df[original_column].max())
        if max_asy >= ASY_H_THRESHOLD_LOW:
            ax.axvline(ASY_H_THRESHOLD_LOW, linestyle="--", color=COLOR_LOW)
            if max_asy >= ASY_H_THRESHOLD_MODERATE:
                ax.axvline(
...
def compute_dtw(y_true, y_pred):
    distances = []
    for true_series, pred_series in zip(y_true, y_pred):
        distance = dtw(true_series, pred_series)
...
def plot_forecast_std(
    ...
    quantile_color=[180 / 255, 225 / 255, 250 / 255, 1],
):
    if ax is None:
        fig, ax = plt.subplots(1, 1, figsize=(13, 6))
    ...
    std_column = df.columns[2]

    month = df.index[len(df) // 2].month_name()
    year = df.index[len(df) // 2].year

    ax.plot(df.index, df[observed_column], color=observed_color, alpha=0.8)

    ax.plot(df.index, df[predicted_column], color=predicted_color, alpha=0.8)

    ax.fill_between(
        df.index,
        df[predicted_column] - df[std_column],
        df[predicted_column] + df[std_column],
        alpha=0.8,
        interpolate=True,
    ...
    rmse_val = rmsem(non_na_df[observed_column], non_na_df[predicted_column])
    r2_val = r2m(non_na_df[observed_column], non_na_df[predicted_column])
    bfe_val = calculate_BFE(non_na_df[observed_column], non_na_df[predicted_column])
    ...
    patch_qbig = mpatches.Patch(color=quantile_color, label=f"interval")
    handles.extend(custom_lines)

    handles.append(patch_qbig)

    labels.extend(["Observed Dst", "Predicted Dst"])
